...
Linux se obnaša podobno kot kateri koli drug UNIX sistem, saj je bil eden izmed glavnih ciljev pri razvoju skladnost s standardi POSIX (\textit{angl. Portable Operating System Interface}) in SUS (\textit{angl. Single UNIX Specification}).
...
SUS oz. Enotna UNIX specifikacija je delo podjetja X/Open. Namen je poenotiti vse UNIX-u podobne sisteme in zagotavljati medsebojno kompatibilnost in prenosljivost kode.
...
Jedro SUS pa je POSIX oz. Prenosni Vmesnik za Operacijske Sisteme, ki ga vzdržuje delovna skupina Austin Group s člani iz ISO, IEEE in The Open Group.
...
Linux API je implementiran preko glibc oz. GNU C knjižnice, ki zagotavlja API vmesnik po standardih ISO C11, POSIX.1-2008 in BSD ter druge OS-specifične API vmesnike.
