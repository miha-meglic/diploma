...
Windows 7 deluje na podlagi client-server arhitekture, ki implementira dve ``osebnosti'', Win32 in POSIX. V tej arhitekturi se uporabniški procesi imenujejo podsistemi (\textit{angl. subsystems}).
Podsistemi so omogočali spremembe in izboljšave na eni izmed ``osebnosti'', brez vpliva na drugo. Kljub podpori za POSIX podsistem pa je Win32 postajal vedno bolj popularen, medtem pa je virtualizacija postala primaren način sobivanja operacijskih sistemov na enem računalniku.
\cite{Silberschatz_Galvin_Gagne_2018}
...
Seveda so se te cilji spreminjali tekom razvoja.\cite{Yosifovich_Russinovich_Solomon_Ionescu_2017}
...
TODO: Nedokumentiran del API-ja (Ntoskrnl.exe, Win32k.sys), Ne-strukturirano procesno drevo, omemba teže NT jedra v imenu kompatibilnosti
